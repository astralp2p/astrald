...
async def list_messages(client, **args) -> list:
    """The envelopes of one of the caller's lists. `args` are the op's own:
    `list`, `from`, `to`, `since`, `unread_only`, `awaiting_pickup`."""
    docs = await jsonops.stream(client, querystring.build(
        "messaging.list_messages", {**args, "out": "json"}))
    return [d["Object"] for d in docs]


async def read(client, *refs) -> dict:
    """A messaging.read_messages_result for (box, id) refs."""
    return await jsonops.value(
        client, "messaging.read_messages?in=json&out=json",
        {"Type": READ_REQUEST,
         "Object": {"Refs": [{"Box": box, "ID": id} for box, id in refs]}})
