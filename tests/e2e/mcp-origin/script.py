...
answered to alpha over apphost, in this same run and on this same node, is what
makes the refusals evidence about the guard rather than about the op or alpha's
permits.

why the exchange is sequential: a message is stored in the recipient's inbox by
the recipient's node (mod/messaging RouteQuery), so beta collects it on its
next wait, and reads the body with read_messages. Neither agent has to be
...
async def main():
    ...
    # same ops of the same node, with only the origin differing from the refusal
