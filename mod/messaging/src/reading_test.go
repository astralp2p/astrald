...
func TestReadingStampsOnlyTheReadersOwnRow(t *testing.T) {
	...
	var sent dbMessage
	if err := mod.db.Where("owner = ? AND box = ?", a, messaging.BoxOutbox).Take(&sent).Error; err != nil {
...
func TestRepliesAreFoundInEitherBox(t *testing.T) {
	...
	ask, answer := messaging.NewMessageID(), messaging.NewMessageID()

	mustInsertOutbox(t, mod, &messaging.StoredMessage{ID: ask, Sender: a, Recipient: b, Content: "which port?"})
	mustInsertInbox(t, mod, &messaging.StoredMessage{ID: ask, Sender: a, Recipient: b, Content: "which port?"})
	mustInsertOutbox(t, mod, &messaging.StoredMessage{ID: answer, Sender: b, Recipient: a, Content: "8626", ParentID: ask})
	mustInsertInbox(t, mod, &messaging.StoredMessage{ID: answer, Sender: b, Recipient: a, Content: "8626", ParentID: ask})

	// A's reply arrived in A's inbox; B's own answer is in B's outbox.
	for _, c := range []struct {
		who *astral.Identity
		box astral.String8
	}{{a, messaging.BoxInbox}, {b, messaging.BoxOutbox}} {
...
func TestAnArchivedReplyIsNotAnsweredAsAChild(t *testing.T) {
	mod := testMessagingModule(t)
	a, b := astral.GenerateIdentity(), astral.GenerateIdentity()
	ask, reply := messaging.NewMessageID(), messaging.NewMessageID()

	mustInsertOutbox(t, mod, &messaging.StoredMessage{ID: ask, Sender: a, Recipient: b, Content: "q"})
	mustInsertInbox(t, mod, &messaging.StoredMessage{ID: reply, Sender: b, Recipient: a, Content: "SECRET", ParentID: ask})

	if n, _ := mod.db.Archive(a, messaging.BoxInbox, reply); n != 1 {
...
func TestAChildsBodyStampsTheRowItCameFrom(t *testing.T) {
	...
	mustInsertOutbox(t, mod, &messaging.StoredMessage{ID: ask, Sender: a, Recipient: b, Content: "q"})
	mustInsertInbox(t, mod, &messaging.StoredMessage{ID: reply, Sender: b, Recipient: a, Content: "the answer", ParentID: ask})

	_, err := mod.readMessages(a, readRequest{
		Refs:     []messageRef{{Box: messaging.BoxOutbox, ID: ask}},
	...
	var row dbMessage
	if err := mod.db.Where("owner = ? AND box = ? AND id = ?", a, messaging.BoxInbox, reply).Take(&row).Error; err != nil {
	...
	unread, err := mod.listMessages(a, messaging.ListMessagesRequest{List: messaging.ListInbox, UnreadOnly: true})
...
// error, not a claim about the participant's own mailbox.
...
// A participant that writes to itself owns both rows of every message, so a
// reply to it matches as a child twice under one id — once as the copy it sent
// and once as the copy it received. It is one reply and is answered once.
func TestASelfSentReplyIsAnsweredOnce(t *testing.T) {
	...
	for _, m := range []struct {
		...
		parent messaging.MessageID
	}{{ask, messaging.MessageID{}}, {reply, ask}} {
		mustInsertOutbox(t, mod, &messaging.StoredMessage{ID: m.id, Sender: a, Recipient: a, Content: "x", ParentID: m.parent})
		mustInsertInbox(t, mod, &messaging.StoredMessage{ID: m.id, Sender: a, Recipient: a, Content: "x", ParentID: m.parent})
	}
	...
	if kids[0].Box != messaging.BoxInbox {
...
// lose the only copy the participant has.
func TestASelfSendThatNeverLandedIsStillAnswered(t *testing.T) {
	mod := testMessagingModule(t)
	a := astral.GenerateIdentity()
	ask, reply := messaging.NewMessageID(), messaging.NewMessageID()

	mustInsertOutbox(t, mod, &messaging.StoredMessage{ID: ask, Sender: a, Recipient: a, Content: "q"})
	mustInsertOutbox(t, mod, &messaging.StoredMessage{ID: reply, Sender: a, Recipient: a, Content: "r", ParentID: ask})
...
func TestAFullAnswerLeavesBodiesAndSaysSo(t *testing.T) {
	...
	for i := range refs {
		id := messaging.NewMessageID()
		mustInsertInbox(t, mod, &messaging.StoredMessage{ID: id, Sender: b, Recipient: a, Content: astral.String32(body)})
		refs[i] = messageRef{Box: messaging.BoxInbox, ID: id}
	}

	res, err := mod.readMessages(a, readRequest{Refs: refs, Children: messaging.ChildrenNone})
	if err != nil {
		t.Fatalf("read: %v", err)
	}
...
func TestRepliesAreChargedAfterTheMessageTheyAnswer(t *testing.T) {
	...
	mustInsertOutbox(t, mod, &messaging.StoredMessage{ID: ask, Sender: a, Recipient: b, Content: astral.String32(body)})
	mustInsertInbox(t, mod, &messaging.StoredMessage{ID: reply, Sender: b, Recipient: a, Content: astral.String32(body), ParentID: ask})
...
func TestAReadsBoundsAreTheModulesAndNotTheCallers(t *testing.T) {
	over := make([]messageRef, messaging.MaxReadRefs+1)
	for i := range over {
		over[i] = messageRef{Box: messaging.BoxInbox, ID: messaging.NewMessageID()}
	}
	if err := (&readRequest{Refs: over}).validate(); err == nil {
		t.Fatalf("naming more than %v ids in one read must be refused", messaging.MaxReadRefs)
	}
	...
	req := readRequest{Refs: over[:1], MaxChildren: messaging.MaxChildren + 500}
	if err := req.validate(); err != nil {
		t.Fatalf("validate: %v", err)
	}
	if req.MaxChildren != messaging.MaxChildren {
		t.Fatalf("max_children was raised to %v past the module's %v", req.MaxChildren, messaging.MaxChildren)
	}
...
func TestAReadNamesEveryReplyEvenPastWhatItCarries(t *testing.T) {
	...
	for range replies {
		id := messaging.NewMessageID()
		mustInsertInbox(t, mod, &messaging.StoredMessage{ID: id, Sender: b, Recipient: a, Content: "r", ParentID: ask})
		want[id.String()] = true
	}

	out, err := mod.readMessages(a, readRequest{
		Refs: []messageRef{{Box: messaging.BoxOutbox, ID: ask}},
	})
	if err != nil {
		t.Fatalf("read: %v", err)
	...
	if len(out.Replies) != messaging.MaxChildren {
		t.Fatalf("the answer carried %v replies, want the module's bound of %v", len(out.Replies), messaging.MaxChildren)
	}

	// every id it named is one it will answer
	refs := make([]messageRef, 0, replies)
	for _, id := range out.Messages[0].ChildIDs {
		refs = append(refs, messageRef{Box: messaging.BoxInbox, ID: id})
	}
	second, err := mod.readMessages(a, readRequest{
		Refs: refs, Children: messaging.ChildrenNone,
	})
	if err != nil {
...
func TestChildIDsComeBackEvenWhenNoRepliesAreAskedFor(t *testing.T) {
	...
	if len(out.Messages[0].ChildIDs) != 1 || out.Messages[0].ChildIDs[0] != reply {
...
func TestNamingOneMessageTwiceReadsItOnce(t *testing.T) {
	...
	a, b := astral.GenerateIdentity(), astral.GenerateIdentity()
	id := messaging.NewMessageID()
	mustInsertInbox(t, mod, &messaging.StoredMessage{ID: id, Sender: b, Recipient: a, Content: "once"})

	ref := messageRef{Box: messaging.BoxInbox, ID: id}
	res, err := mod.readMessages(a, readRequest{Refs: []messageRef{ref, ref, ref}, Children: messaging.ChildrenNone})
	if err != nil {
		t.Fatalf("read: %v", err)
	}
	...
	repeated := make([]messageRef, messaging.MaxReadRefs+5)
