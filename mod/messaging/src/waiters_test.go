...
func TestADeliveryWakesTheParkedWait(t *testing.T) {
	...
	if err := mod.storeMessage(b, a, &messaging.Message{ID: messaging.NewMessageID(), Content: "x"}); err != nil {
...
// A wake names one mailbox. Waking every parked participant would cost a query
// each, measured at two hundred to one.
...
func TestUndoingAnArchiveWakesTheWait(t *testing.T) {
	...
	if _, err := mod.archiveMessage(a, messageRef{Box: messaging.BoxInbox, ID: id}, false); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := mod.archiveMessage(a, messageRef{Box: messaging.BoxInbox, ID: id}, true); err != nil {
		t.Fatal(err)
	}
...
func TestARetryThatStoredNothingWakesNobody(t *testing.T) {
	mod := testMessagingModule(t)
	a, b := astral.GenerateIdentity(), astral.GenerateIdentity()
	msg := &messaging.Message{ID: messaging.NewMessageID(), Content: "x"}

	if err := mod.storeMessage(b, a, msg); err != nil {
		t.Fatal(err)
	}
	if _, err := mod.archiveMessage(a, messageRef{Box: messaging.BoxInbox, ID: msg.ID}, false); err != nil {
		t.Fatal(err)
	}
...
func TestEveryExitPathUnregisters(t *testing.T) {
	...
	t.Run("refused before parking", func(t *testing.T) {
		if _, err := mod.waitMessages(context.Background(), c, waitRequest{Since: math.MaxUint64}); err == nil {
