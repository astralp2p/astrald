...
// waiters is the set of parked waits, keyed by the identity whose mailbox each
...
// per parked wait for every arrival.
//
// why the send never blocks: the caller still owes its sender an Ack, and
// holding it until the delivery timeout makes a stored message read as one whose
// fate is unknown.
