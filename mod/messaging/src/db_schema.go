...
// The module's tables, named here once for the statements that cannot take a
// gorm model.
const (
	tableMailboxes = "messaging__mailboxes"
	tableMessages  = "messaging__messages"
)
...
// guarantee and OnConflict resolves against it. It carries box because one
// participant may hold two rows under one id, sending to itself or receiving an
// id it sent.
//
...
const ddlMessages = `
CREATE TABLE IF NOT EXISTS messaging__messages (
...
var ddlIndexes = []string{
	`CREATE UNIQUE INDEX IF NOT EXISTS ux_messaging__messages ON messaging__messages (owner, box, id)`,
	`CREATE INDEX IF NOT EXISTS ix_messaging__messages_box ON messaging__messages (owner, box, archived_at, seq)`,
	`CREATE INDEX IF NOT EXISTS ix_messaging__messages_parent ON messaging__messages (owner, parent_id, archived_at, created_at)`,
