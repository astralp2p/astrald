...
func (mod *Module) noteFetched(m *messaging.StoredMessage) {
	// why the box is checked first: only handing out an inbox body is a
	// collection. Stamping a participant's own sent row would tell it that
	// someone collected a message it merely re-read.
	if m.Box != messaging.BoxInbox {
		return
	}

	if mod.hosts(m.Sender) {
	...
	go func(recipient, sender *astral.Identity, id messaging.MessageID) {
...
//
// why the context is the node's: see deliverMessage.
func (mod *Module) sendReceipt(recipientID, senderID *astral.Identity, id messaging.MessageID) error {
	qctx, cancel := mod.ctx.WithTimeout(mod.config.DeliveryTimeout)
	defer cancel()

	conn, err := query.RouteInFlight(qctx, mod.node, launchDelivery(recipientID, senderID, messaging.MethodReceipt))
	if err != nil {
		return err
	}
	defer conn.Close()
	...
	if err = ch.Send(&messaging.Receipt{ID: id}); err != nil {
		return err
	}
...
//
// The row is the admission. A caller naming a message this participant never
// sent it is answered an error, and a node holding no such row learns nothing
// about why.
