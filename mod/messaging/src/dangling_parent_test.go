...
func TestAReplyToAHeldInboxMessageIsStored(t *testing.T) {
	...
	mustInsertInbox(t, mod, &messaging.StoredMessage{ID: ask, Sender: b, Recipient: a, Content: "ask"})

	reply := messaging.NewMessageID()
	if err := mod.storeMessage(b, a, &messaging.Message{ID: reply, Content: "reply", ParentID: ask}); err != nil {
...
func TestAReplyToAHeldOutboxMessageIsStored(t *testing.T) {
	...
	ask := messaging.NewMessageID()
	mustInsertOutbox(t, mod, &messaging.StoredMessage{ID: ask, Sender: a, Recipient: b, Content: "ask"})
...
func TestAReplyToAnArchivedParentIsStored(t *testing.T) {
	...
	ask := messaging.NewMessageID()
	mustInsertInbox(t, mod, &messaging.StoredMessage{ID: ask, Sender: b, Recipient: a, Content: "ask"})
	if n, _ := mod.db.Archive(a, messaging.BoxInbox, ask); n != 1 {
...
func TestAWireCycleIsRefusedAtTheFirstEdge(t *testing.T) {
	...
	x, y := messaging.NewMessageID(), messaging.NewMessageID()

	// x answers y, which does not exist yet: refused
	if err := mod.storeMessage(b, a, &messaging.Message{ID: x, Content: "x", ParentID: y}); err == nil {
	...
	if err := mod.storeMessage(b, a, &messaging.Message{ID: y, Content: "y", ParentID: x}); err == nil {
...
func TestARootMessageIsAlwaysStored(t *testing.T) {
	...
	if err := mod.storeMessage(b, a, &messaging.Message{ID: id, Content: "root"}); err != nil {
...
func TestARedeliveredReplyIsStillOneRow(t *testing.T) {
	...
	reply := messaging.NewMessageID()
	msg := &messaging.Message{ID: reply, Content: "reply", ParentID: ask}
	...
	rows, err := mod.db.ListMessages(a, messageQuery{List: messaging.ListInbox, From: b})
	if err != nil {
		t.Fatal(err)
	}
	count := 0
...
// A send refuses a reply to a message the sender does not hold, before any
// outbox row is written.
func TestSendRefusesAReplyToAMessageNotHeld(t *testing.T) {
	mod := testMessagingModule(t)
	sender := hostedParticipant(t, mod)
	peer := astral.GenerateIdentity()
	mod.Dir.(*stubDir).aliases["peer"] = peer

	stranger := messaging.NewMessageID()
	_, err := mod.sendMessage(sender, "peer", "reply", stranger)
	if err == nil {
		t.Fatal("a reply to a message the sender does not hold must be refused")
	}
