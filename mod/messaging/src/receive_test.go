...
func TestRouteQueryStoresMessage(t *testing.T) {
	...
	recipient := hostedParticipant(t, mod)

	obj := deliverOverRouter(t, mod, recipient, &messaging.Message{
		ID:      testID(1),
	...
	rows, _, err := mod.db.FindMany(recipient, []messageRef{{Box: messaging.BoxInbox, ID: testID(1)}})
	if err != nil {
...
func TestRouteQueryRefusesOversizeMessage(t *testing.T) {
	...
	rows, err := mod.listMessages(recipient, messaging.ListMessagesRequest{List: messaging.ListInbox})
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	if len(rows) != 0 {
