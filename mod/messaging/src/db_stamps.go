...
func (db *DB) Archive(owner *astral.Identity, box string, id messaging.MessageID) (int64, error) {
...
// Unarchive clears the stamp. Archive is the participant's own bookkeeping and
// crosses no link, so it is the one stamp here with an inverse.
func (db *DB) Unarchive(owner *astral.Identity, box string, id messaging.MessageID) (int64, error) {
	tx := db.Model(&dbMessage{}).
...
func (db *DB) MarkReceiptDue(recipient *astral.Identity, id messaging.MessageID) (int64, error) {
	tx := db.Model(&dbMessage{}).
		Where("owner = ? AND box = ? AND id = ? AND receipt_due_at IS NULL",
			recipient, messaging.BoxInbox, id).
		Update("receipt_due_at", time.Now().UTC())
	return tx.RowsAffected, tx.Error
}

// StampReceiptStored records the sender's node acknowledging our receipt.
func (db *DB) StampReceiptStored(recipient *astral.Identity, id messaging.MessageID) error {
...
func (db *DB) StampLanded(sender *astral.Identity, id messaging.MessageID) error {
	return db.Model(&dbMessage{}).
		Where("owner = ? AND box = ? AND id = ? AND landed_at IS NULL",
			sender, messaging.BoxOutbox, id).
		Update("landed_at", time.Now().UTC()).Error
}

// StampFailed records a delivery known not to have been stored.
func (db *DB) StampFailed(sender *astral.Identity, id messaging.MessageID) error {
	return db.Model(&dbMessage{}).
...
func (db *DB) StampFetched(sender *astral.Identity, id messaging.MessageID) error {
	return db.Model(&dbMessage{}).
...
func (db *DB) StampFetchedFrom(sender, recipient *astral.Identity, id messaging.MessageID) (int64, error) {
	tx := db.Model(&dbMessage{}).
		Where("owner = ? AND box = ? AND id = ? AND recipient = ? AND fetched_at IS NULL",
			sender, messaging.BoxOutbox, id, recipient).
...
func (db *DB) SetErr(sender *astral.Identity, id messaging.MessageID, text string) error {
	return db.Model(&dbMessage{}).
		Where("owner = ? AND box = ? AND id = ? AND err IS NULL", sender, messaging.BoxOutbox, id).
