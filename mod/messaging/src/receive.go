...
//
// why the node answers and not the participant: storing a message is a write,
// and it finishes inside the resolve deadline. The recipient's model is not part
// of delivery and need not be running.
func (mod *Module) acceptMessage(q *astral.InFlightQuery, w io.WriteCloser) (io.WriteCloser, error) {
	...
	return query.Accept(q, w, func(conn astral.Conn) {
		...
		msg, ok := obj.(*messaging.Message)
		if !ok {
			_ = ch.Send(astral.NewError("not a message"))
			return
		}

		// why under whileIndexed: hosting was asked when the delivery was
		// accepted, and the mailbox may have been withdrawn since.
		err = mod.whileIndexed(recipient, func() error {
			return mod.storeMessage(sender, recipient, msg)
		})
		if err != nil {
...
// name someone else.
func (mod *Module) storeMessage(sender, recipient *astral.Identity, msg *messaging.Message) error {
	switch {
	case msg.ID.IsZero():
	...
	n, err := mod.db.InsertInbox(&messaging.StoredMessage{
		ID:        msg.ID,
		Sender:    sender,
		Recipient: recipient,
		Content:   msg.Content,
	...
	return mod.checkRetry(sender, recipient, msg.ID)
}

// checkRetry answers a delivery that wrote nothing. That is either a sender
// repeating a delivery whose acknowledgement it never saw or a second sender
// minting an id this inbox already holds. The id is the sender's to choose, so
// the second is reachable and must not be answered with an ack.
func (mod *Module) checkRetry(sender, recipient *astral.Identity, id messaging.MessageID) error {
	held, err := mod.db.SenderOf(recipient, messaging.BoxInbox, id)
	if err != nil {
		return err
	}
