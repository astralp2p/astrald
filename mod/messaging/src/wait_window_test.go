...
func TestATimedOutAnswerKeepsTheCursor(t *testing.T) {
	...
	first, err := mod.Wait(context.Background(), owner, messaging.WaitRequest{}, nil)
	if err != nil {
		t.Fatal(err)
	}
	if first.TimedOut || first.NextSince == 0 {
		t.Fatalf("first wait: timed_out %v, next_since %v", first.TimedOut, first.NextSince)
	}
	if time.Duration(first.Granted) != defaultConfig.WaitDefault {
		t.Fatalf("granted: got %v, want the default %v", first.Granted, defaultConfig.WaitDefault)
	}

	again, err := mod.Wait(context.Background(), owner, messaging.WaitRequest{
		Since:   uint64(first.NextSince),
		Timeout: time.Second,
	}, nil)
	if err != nil {
		t.Fatal(err)
	...
	if again.NextSince != first.NextSince {
		t.Fatalf("cursor: got %v, want the one sent, %v", again.NextSince, first.NextSince)
	}
	if time.Duration(again.Granted) != time.Second || time.Duration(again.Waited) < time.Second {
		t.Fatalf("window: granted %v waited %v, want 1s and at least 1s", again.Granted, again.Waited)
	}
}
