...
func mustInsertInbox(t *testing.T, mod *Module, m *messaging.StoredMessage) {
	t.Helper()
...
func mustInsertOutbox(t *testing.T, mod *Module, m *messaging.StoredMessage) {
	t.Helper()
...
func TestSchemaCarriesWhatTheDDLDeclares(t *testing.T) {
	...
	if err := mod.db.Raw(`SELECT name, hidden FROM pragma_table_xinfo('messaging__messages')`).Scan(&cols).Error; err != nil {
	...
	if err := mod.db.Raw(`SELECT name, "unique", partial FROM pragma_index_list('messaging__messages')`).Scan(&idx).Error; err != nil {
		t.Fatalf("index_list: %v", err)
	}

	want := map[string][2]int{
		"ux_messaging__messages":         {1, 0},
		"ix_messaging__messages_box":     {0, 0},
		"ix_messaging__messages_parent":  {0, 0},
		"ix_messaging__messages_archive": {0, 1},
		"ix_messaging__messages_unread":  {0, 1},
		"ix_messaging__messages_pickup":  {0, 1},
	}
...
func TestOwnerIsGeneratedAndUnwritable(t *testing.T) {
	...
	}{{messaging.BoxOutbox, a}, {messaging.BoxInbox, b}} {
		var row dbMessage
	...
	err := mod.db.Exec(`UPDATE messaging__messages SET owner = ? WHERE id = ?`, b, id).Error
	if err == nil {
...
func TestTheBoxChecksRefuseACrossBoxWrite(t *testing.T) {
	...
	if err := mod.db.Exec(`UPDATE messaging__messages SET landed_at = ? WHERE id = ?`, "2026-01-01", id).Error; err == nil {
		t.Fatal("landed_at on an inbox row must be refused")
	}
	if err := mod.db.Exec(`INSERT INTO messaging__messages (box,id,sender,recipient,content,created_at) VALUES ('archive',?,?,?,'x','2026-01-01')`, messaging.NewMessageID(), a, b).Error; err == nil {
...
// A participant writing to itself holds both rows, and a peer may mint an id
// this participant already sent under. The id is the peer's to choose, so
// neither is refusable — and both are why every statement names the box as well
// as the owner.
func TestOneOwnerHoldsTwoRowsUnderOneID(t *testing.T) {
	...
	mustInsertOutbox(t, mod, &messaging.StoredMessage{ID: id, Sender: a, Recipient: astral.GenerateIdentity(), Content: "sent"})
	mustInsertInbox(t, mod, &messaging.StoredMessage{ID: id, Sender: c, Recipient: a, Content: "a peer minted the same id"})
...
// The box is half of what names a row, so a participant that holds both rows of
// one id named two messages rather than one twice.
func TestTheSameIDInBothBoxesIsTwoMessages(t *testing.T) {
	mod := testMessagingModule(t)
	a := astral.GenerateIdentity()
	id := messaging.NewMessageID()
	mustInsertOutbox(t, mod, &messaging.StoredMessage{ID: id, Sender: a, Recipient: a, Content: "sent"})
	mustInsertInbox(t, mod, &messaging.StoredMessage{ID: id, Sender: a, Recipient: a, Content: "received"})

	res, err := mod.readMessages(a, readRequest{
		Refs:     []messageRef{{Box: messaging.BoxOutbox, ID: id}, {Box: messaging.BoxInbox, ID: id}},
...
func TestStoreRefusesASelfParentAndADanglingOne(t *testing.T) {
	...
	id := messaging.NewMessageID()
	err := mod.storeMessage(a, b, &messaging.Message{ID: id, Content: "x", ParentID: id})
	if err == nil {
		t.Fatal("a message answering itself must be refused")
	}

	// a parent this node does not hold is refused, and nothing is stored
	stranger := messaging.NewMessageID()
	child := messaging.NewMessageID()
	if err := mod.storeMessage(a, b, &messaging.Message{ID: child, Content: "x", ParentID: stranger}); err == nil {
...
func TestStoreTellsARetryFromACollision(t *testing.T) {
	...
	if err := mod.storeMessage(a, b, &messaging.Message{ID: id, Content: "first"}); err != nil {
		t.Fatalf("first delivery: %v", err)
	}
	if err := mod.storeMessage(a, b, &messaging.Message{ID: id, Content: "first"}); err != nil {
		t.Fatalf("the same sender retrying must be accepted: %v", err)
	}
	if err := mod.storeMessage(c, b, &messaging.Message{ID: id, Content: "hijack"}); err == nil {
...
// A remote node's words land in this participant's sent list and from there in
// its model's context. They are quoted material, but how much of a context
// window a refusing peer occupies is this node's decision, taken where the row
// is written — the only place that bounds them.
func TestARemoteRefusalIsBoundedInTheStore(t *testing.T) {
	mod := testMessagingModule(t)
	sender, recipient := astral.GenerateIdentity(), astral.GenerateIdentity()
	id := messaging.NewMessageID()

	mustInsertOutbox(t, mod, &messaging.StoredMessage{ID: id, Sender: sender, Recipient: recipient, Content: "x"})
	...
	if len(got) > errLimit+len("… (cut)") {
		t.Fatalf("a refusing node wrote %v bytes into the participant's context", len(got))
	}
	...
	if err = mod.db.SetErr(sender, messaging.NewMessageID(), "refused"); err != nil {
		t.Fatal(err)
	}
}
