...
// dbMessage is one message in one participant's box. Every message a node
// carries is two rows, the sender's and the recipient's, differing in box and
// owner and in nothing else; across nodes only one of them is on this machine.
type dbMessage struct {
	...
	// ArchivedAt is the participant putting a message away: the only disposal
	// marker, on either box, and the only one with an inverse. It crosses no
	// link.
...
func (dbMessage) TableName() string {
	return messagingmod.DBPrefix + "messages"
}
...
func (row dbMessage) stored() *messaging.StoredMessage {
	m := &messaging.StoredMessage{
...
func storedAll(rows []dbMessage) []*messaging.StoredMessage {
	list := make([]*messaging.StoredMessage, len(rows))
...
// stamps for itself.
func newRow(m *messaging.StoredMessage) *dbMessage {
	return &dbMessage{
		ID:        m.ID,
		Sender:    m.Sender,
