...
func (db *DB) InsertOutbox(m *messaging.StoredMessage) error {
	row := newRow(m)
	row.Box = messaging.BoxOutbox
...
func (db *DB) InsertInbox(m *messaging.StoredMessage) (int64, error) {
	row := newRow(m)
	row.Box = messaging.BoxInbox
	row.CreatedAt = time.Now().UTC()
...
func (db *DB) FindMany(owner *astral.Identity, refs []messageRef) (list []*messaging.StoredMessage, missing []messageRef, _ error) {
	for _, ref := range refs {
		...
		// falsehood about the participant's own mailbox.
...
func (db *DB) MarkRead(owner *astral.Identity, m *messaging.StoredMessage) error {
	if m.Box != messaging.BoxInbox || m.ReadAt != nil {
		return nil
	}
	now := time.Now().UTC()
	err := db.Model(&dbMessage{}).
		Where("owner = ? AND box = ? AND id = ? AND read_at IS NULL", owner, messaging.BoxInbox, m.ID).
...
//
// why a self-send collapses to its inbox row: a participant that writes to
// itself owns both rows, so a reply matches twice under one id. The inbox copy
// carries the read stamp, and it is kept only where it exists.
func childrenOf(db *gorm.DB, owner *astral.Identity, parent messaging.MessageID) *gorm.DB {
	return db.Where("owner = ? AND parent_id = ? AND archived_at IS NULL", owner, parent).
		Where(`NOT (box = 'outbox' AND EXISTS (
			SELECT 1 FROM messaging__messages self
			WHERE self.owner = messaging__messages.owner
			  AND self.box = 'inbox'
			  AND self.id = messaging__messages.id))`)
}
...
func (db *DB) Children(owner *astral.Identity, parent messaging.MessageID, limit int) ([]*messaging.StoredMessage, error) {
	var rows []dbMessage
...
func (db *DB) ChildIDs(owner *astral.Identity, parent messaging.MessageID) (ids []messaging.MessageID, _ error) {
...
func (db *DB) SenderOf(owner *astral.Identity, box string, id messaging.MessageID) (*astral.Identity, error) {
	var row dbMessage
...
func (db *DB) Holds(owner *astral.Identity, id messaging.MessageID) (bool, error) {
	err := db.Select("seq").
		Where("owner = ? AND box IN (?, ?) AND id = ?",
			owner, messaging.BoxInbox, messaging.BoxOutbox, id).
