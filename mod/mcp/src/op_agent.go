...
// agent metadata and no credential, so the token is left out of the object.
// The origin refusal below narrows who reaches the guard and identifies no
// caller; see OpListAgents for what the refusal covers.
