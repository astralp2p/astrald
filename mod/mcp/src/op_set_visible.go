...
type opSetVisibleArgs struct {
	ID      string `query:"required"`
	Visible bool   `query:"required"`
	Out     string
}

// OpSetVisible opens or closes an agent to callers other than itself. ID takes
// an identity or an alias.
//
// why local-only: visibility is the account holder's decision, made through the
...
func (mod *Module) OpSetVisible(ctx *astral.Context, q *routing.IncomingQuery, args opSetVisibleArgs) error {
	...
	if err = mod.db.SetVisible(agentID, args.Visible); err != nil {
	...
	if args.Visible {
		...
		_ = mod.visible.Remove(agentID.String())
	...
	mod.log.Logv(1, "agent %v visible=%v", agentID, args.Visible)
