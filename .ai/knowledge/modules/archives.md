...
| `auth` | recurses into the parent archive's `SeeObjectsAction` to decide entry access |
...
* `AuthorizeSeeObjects` recursively delegates to the parent archive's `SeeObjectsAction`; it returns false when the action names no object, when the parent row is missing, or when the entry ID equals the parent ID (self-reference guard).
